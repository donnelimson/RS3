...
angular.module("MetronicApp")
    ...
    .controller('GoodsReceiptCreateController', ['$scope', 'GoodsReceiptService', 'NgTableParams', '$uibModal', '$window','hotkeys','CommonService',
        function ($scope, GoodsReceiptService, NgTableParams, $uibModal, $window,hotkeys,CommonService) {
            ...
            function getGoodsReceiptIdForShortCut() {
                GoodsReceiptService.GetGoodsRecieptIdForShortcutManual().
                then(function (data) {
            ...
            hotkeys.bindTo($scope).add({
                combo: 'shift+s',
                callback: function () {
                    $scope.saveCreatedGoodsReceipt($scope.goodsReceiptId);
                }
            });
            ...
            $scope.saveCreatedGoodsReceipt = function (goodsReceiptId) {
                ...
                if ($scope.proceed == true) {
                    swal({
                        ...
                        GoodsReceiptService.SaveCreatedGoodsReceipt({
            ...
            $scope.completeGoodsReceipt = function (goodsReceiptId, supplierId) {
                ...
                for (var i = 0; i <= $scope.dataToInsert.length - 1; i++) {
                    if ($scope.dataToInsert[i].Quantity < 1 || $scope.dataToInsert[i].Cost < 1) {
                        ...
                        $scope.proceed = true;
                        $scope.saveCreatedGoodsReceipt(goodsReceiptId);
                    }
                }
            ...
            $scope.addItem = function (supplierId) {
                ...
                $uibModal.open({
                    ...
                    for (var i = 1; i <= data.length - 1; i++) {
                        if($scope.dataToInsert[0])
                        $scope.dataToInsert.push({ SupplierId: $scope.dataToInsert[0].SupplierId, SupplierDescription: $scope.dataToInsert[0].SupplierDescription, MasterItemId: data[i].MasterItemId, MasterItemDescription: data[i].Description, Quantity: data[i].Quantity, Cost: data[i].Cost, TotalCost: data[i].Cost * data[i].Quantity });
                    }
                    computation();
            ...
            $scope.addItemForCreate = function () {
                ...
                if ($scope.supplierId == 0) {
                    CommonService.warningMessage("Please select supplier first")
                }
                else {
                    ...
                    $uibModal.open({
                        ...
                        for (var i = 1; i <= data.length - 1; i++) {
                            $scope.dataToInsert.push({ SupplierId: $scope.supplierId, MasterItemId: data[i].MasterItemId, MasterItemDescription: data[i].Description, Quantity: data[i].Quantity, Cost: parseFloat(data[i].Cost.toFixed(2)), TotalCost: data[i].Cost * data[i].Quantity });
                        }
                        computation();
                        $scope.formSubmitted = false;
                    }, function () {
