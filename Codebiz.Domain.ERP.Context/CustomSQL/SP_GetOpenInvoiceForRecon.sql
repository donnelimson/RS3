...
if exists(select 1 from sys.sysobjects where [name] = 'SP_GetOpenInvoiceForRecon' and xtype = 'P')
	drop procedure SP_GetOpenInvoiceForRecon
go

create procedure SP_GetOpenInvoiceForRecon
...
as
begin 
--exec SP_GetOpenInvoiceForRecon @cardCode = 'V10001', @dateType = 'P' , @fromDate = '2021-01-01', @toDate = '2021-12-31'
	select  
	...
	from (
			...
			where 
			case 
				when @cardCode = '' then 1 
				when @cardCode <> ''  and JEN1.ShortName = @cardCode then 1 
				else 0 
			end = 1
			and 
				case 
